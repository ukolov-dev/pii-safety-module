"""Production detector v3.3: ownership-gated PII detection with broader Russian syntax.
...
from .detector_v3_2 import detect as detect_v3_2
...
_V3_3_TRIGGER = re.compile(
    r"(?:фамили|ф\.?\s*и\.?\s*о|на\s+имя|согласи|заявк|заявлени|"
    r"меня\s+зовут|анкет|получател|заявител|клиент|ИНН|паспорт|"
    r"код\s+подраздел|дата\s+рожд|родил|рожд[её]н|место\s+рожд|"
    r"гражданин|когда\s+выдан|дата\s+выдачи|выдали|выдан|"
    r"водитель|права|в\s*/\s*у|CVV|CVC|CARD\s*HOLDER|держател|"
    r"владелец|имя\s+на\s+карте|адрес|регистрац|я\s+живу)",
    re.IGNORECASE,
)
...
def _same_clause(text: str, start: int, end: int, pattern: re.Pattern[str]) -> bool:
    """Match a semantic cue without leaking it into a neighbouring sentence."""

    left = 0
    for boundary in re.finditer(r"(?:[;!?]|\.(?=\s+[A-ЯЁA-Z]))\s*", text[:start]):
        left = boundary.end()
    right_match = re.search(r"(?:[;!?]|\.(?=\s+[A-ЯЁA-Z]))", text[end:])
...
def _new_candidates(text: str) -> Iterator[DetectedEntity]:
    # v3.3 recognizers are semantic and label-driven. Avoid dozens of complete
    # regex passes over very large payloads when none of their anchors exists.
    if _V3_3_TRIGGER.search(text) is None:
        return
    ...
    for match in _NUMERIC_DATE.finditer(text):
        if not _valid_numeric_date(match) or _near(text, *match.span(), _DOC_CUE):
            continue
        if _same_clause(text, *match.span(), _BIRTH_CUE):
            yield _entity(text, "BIRTH_DATE", *match.span(), "birth_context_v3_3", 97)
        elif _same_clause(text, *match.span(), _ISSUE_CUE) or _same_clause(
            text, *match.span(), _PASSPORT_NEAR
        ):
    ...
    for match in _WORD_DATE.finditer(text):
        ...
        elif _same_clause(text, *match.span(), _ISSUE_CUE):
