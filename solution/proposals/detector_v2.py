"""Candidate v2: deterministic Russian PII recognizers with labelled-field coverage.

This module is deliberately isolated from ``app.detection.detector``.  It duplicates
the production recognizers and adds conservative, label-anchored patterns so the
candidate can be benchmarked before it is promoted.
"""
...
_RU_TITLE_WORD = r"[А-ЯЁ][а-яё]+(?:-[А-ЯЁ][а-яё]+)?"
_RU_UPPER_WORD = r"[А-ЯЁ]{2,}(?:-[А-ЯЁ]{2,})?"
_CONTEXT_FIO_RE = re.compile(
    rf"\b(?i:фио|клиент|гражданин|владелец|получатель|заявитель)\s*[:—-]?\s*"
    rf"(?P<name>(?:{_RU_TITLE_WORD}|{_RU_UPPER_WORD})"
    rf"(?:\s+(?:{_RU_TITLE_WORD}|{_RU_UPPER_WORD})){{1,2}})\b"
)
_PATRONYMIC_FIO_RE = re.compile(
    rf"(?<![А-Яа-яЁё-])(?P<name>{_RU_TITLE_WORD}\s+{_RU_TITLE_WORD}\s+"
    rf"[А-ЯЁ][а-яё]+(?:ович|евич|ич|овна|евна|ична|инична))\b"
)

_PASSPORT_CONTEXT = re.compile(r"\b(?:паспорт|паспорта|серия|серии)\b", re.IGNORECASE)
_BIRTH_CONTEXT = re.compile(
    r"\b(?:дата\s+рождения|родил(?:ся|ась)|год\s+рождения|д\.?\s*р\.?)\b", re.IGNORECASE
)
_DIVISION_CONTEXT = re.compile(r"\bкод\s+подразделения\b", re.IGNORECASE)

# Candidate-only recognizers. Values end at sentence punctuation and are accepted only
# after explicit field labels. This intentionally avoids free-form address extraction.
_LABELLED_PATTERNS: tuple[tuple[str, re.Pattern[str], float, int], ...] = (
    (
        "PLACE_OF_BIRTH",
        re.compile(
            r"\bместо\s+рождения\s*:\s*"
            r"(?P<value>(?:г(?:ород|\.)\s+)?[А-ЯЁ][А-Яа-яЁё -]*?)(?=[.;,]|$)",
            re.IGNORECASE,
        ),
        0.96,
        90,
    ...
    (
        "CITIZENSHIP",
        re.compile(
            r"\bгражданство\s*:\s*(?P<value>[А-ЯЁ][А-Яа-яЁё -]*?)(?=[.;,]|$)",
            re.IGNORECASE,
        ),
        0.97,
        90,
    ...
    (
        "DRIVER_LICENSE_RF",
        re.compile(
            r"\bводительское\s+удостоверение\s*:\s*"
    ...
    (
        "ADDRESS_COUNTRY",
        re.compile(
            r"\bстрана\s+проживания\s*:\s*"
            r"(?P<value>[А-ЯЁ][А-Яа-яЁё -]*?)(?=[.;,]|$)",
            re.IGNORECASE,
        ),
        0.97,
        90,
    ...
    (
        "ADDRESS_POSTAL_CODE",
        re.compile(
            r"\bпочтовый\s+индекс(?:\s+клиента)?\s*:\s*"
            r"(?P<value>\d{6})(?!\d)",
            re.IGNORECASE,
        ),
        0.99,
    ...
    (
        "ADDRESS_HOUSE",
        re.compile(
            r"\bдом(?:\s+клиента)?\s*:\s*"
            r"(?P<value>\d+[A-Za-zА-Яа-яЁё]?"
            r"(?:[/ -]\d+[A-Za-zА-Яа-яЁё]?)?)(?=[.;,]|$)",
            re.IGNORECASE,
        ),
        0.97,
    ...
    (
        "CARDHOLDER_NAME",
        re.compile(
            r"\bимя\s+держателя\s+карты\s*:\s*"
            r"(?P<value>[A-Z]{2,}(?:[ -][A-Z]{2,}){1,3})(?=[.;,]|$)",
            re.IGNORECASE,
        ),
        0.98,
        92,
...
def _candidates(text: str) -> Iterator[DetectedEntity]:
    for match in _EMAIL_RE.finditer(text):
        yield _entity(text, "EMAIL", *match.span(), 0.99, "email_regex_v2", 80)
    ...
    for pattern in (_NUMERIC_DATE_RE, _TEXT_DATE_RE):
    ...
    for match in _CONTEXT_FIO_RE.finditer(text):
        ...
        yield _entity(text, "PERSON", start, end, 0.86, "russian_name_context_v2", 70)
    ...
    for entity_type, pattern, confidence, priority in _LABELLED_PATTERNS:
        for match in pattern.finditer(text):
            start, end = match.span("value")
            yield _entity(text, entity_type, start, end, confidence, "labelled_field_v2", priority)
