"""Compare production v3.13 and proposal v3.19 on disclosed datasets through v12."""
...
from proposals.detector_v3_19 import detect as detector_v3_19
...
DISCLOSED_DATASETS = (
    ...
    *(f"sealed_holdout_v{version}.json" for version in range(4, 13)),
)
...
def compare_all(root: Path) -> dict[str, dict[str, dict[str, int | float]]]:
    detectors = {"v3.13": detector_v3_13, "v3.19": detector_v3_19}
    return {
        filename: {
            name: score(json.loads((root / filename).read_text(encoding="utf-8")), detector)
