"""Compare production v3.20 and proposal v3.21 through disclosed v13."""
...
from app.detection.detector_v3_20 import detect as detector_v3_20
from app.detection.models import DetectedEntity
from benchmark.evaluate_quality import resolve_gold
from proposals.detector_v3_21 import detect as detector_v3_21
...
DISCLOSED_DATASETS = (
    ...
    *(f"sealed_holdout_v{version}.json" for version in range(4, 14)),
)
...
def compare_all(root: Path) -> dict[str, dict[str, dict[str, int | float]]]:
    detectors = {"v3.20": detector_v3_20, "v3.21": detector_v3_21}
    ...
    for filename in DISCLOSED_DATASETS:
        ...
        result[filename] = {name: score(dataset, detector) for name, detector in detectors.items()}
    return result
