...
DATASET_PATH = Path(__file__).parents[2] / "benchmark" / "sealed_holdout_v14.json"
...
def test_sealed_holdout_v14_is_structurally_valid_and_balanced() -> None:
    dataset = _load()
    cases = dataset["cases"]
    ...
    assert dataset["name"] == "ru_pii_sealed_holdout_v14"
    assert dataset["version"] == 14
    assert len(cases) == 140
    assert len(positives) == len(negatives) == 70
