...
DATASET_PATH = Path(__file__).parents[2] / "benchmark" / "sealed_holdout_v16.json"
...
def test_sealed_holdout_v16_is_structurally_valid_and_balanced() -> None:
    dataset = _load()
    cases = dataset["cases"]
    ids = [case["id"] for case in cases]
    positives = [case for case in cases if case["entities"]]
    negatives = [case for case in cases if not case["entities"]]
    assert dataset["name"] == "ru_pii_sealed_holdout_v16"
    assert dataset["version"] == 16
    assert len(cases) == 180
    assert len(positives) == len(negatives) == 90
    assert len(ids) == len(set(ids))
    assert len({entity["type"] for case in positives for entity in case["entities"]}) == 22
    for case in cases:
