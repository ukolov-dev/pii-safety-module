...
DATASET_PATH = Path(__file__).parents[2] / "benchmark" / "sealed_holdout_v6.json"
...
def test_sealed_holdout_v6_is_structurally_valid_and_balanced() -> None:
    dataset = _load()
    cases = dataset["cases"]
    ...
    assert dataset["name"] == "ru_pii_sealed_holdout_v6"
    assert dataset["version"] == 6
    assert 80 <= len(cases) <= 100
    assert abs(len(positives) - len(negatives)) <= 10
