...
DATASET_PATH = Path(__file__).parents[2] / "benchmark" / "sealed_holdout_v9.json"
...
def test_sealed_holdout_v9_is_structurally_valid_and_balanced() -> None:
    dataset = _load()
    cases = dataset["cases"]
    ...
    assert dataset["name"] == "ru_pii_sealed_holdout_v9"
    assert dataset["version"] == 9
