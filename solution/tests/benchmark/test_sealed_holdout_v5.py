...
DATASET_PATH = Path(__file__).parents[2] / "benchmark" / "sealed_holdout_v5.json"
...
def test_sealed_holdout_v5_is_structurally_valid() -> None:
    dataset = _load()
    cases = dataset["cases"]
    case_ids = [case["id"] for case in cases]

    assert dataset["name"] == "ru_pii_sealed_holdout_v5"
